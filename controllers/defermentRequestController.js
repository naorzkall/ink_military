...
exports.submitDefermentRequest = async (req, res, next) => {
  ...
  try {
    ...
    const identityFile = req.files["identity"]
      ? req.files["identity"][0]
      : null;
    ...
    if (!identityFile || !certificateFile) {
      return res
        .status(400)
        .send("Identity and certificate files are required.");
    }
...
exports.viewRequest = async (req, res) => {
  try {
    ...
    const request = await DefermentRequest.findById(requestId).populate(
      "userId"
    );
...
exports.addRequestToCart = async (req, res) => {
  try {
    ...
    if (!request) {
      return res
        .status(404)
        .send("Request not found or already being processed");
    }
...
exports.approveRequest = async (req, res) => {
  try {
    ...
    if (
      req.files &&
      req.files.defermentDocument &&
      req.files.defermentDocument[0]
    ) {
    ...
    if (!request) {
      ...
      return res
        .status(404)
        .send("Request not found or not in processing state");
    }

    await Student.updateOne({ _id: request.userId }, [
      {
        $set: {
...
exports.processRequest = async (req, res) => {
  try {
    ...
    const request = await DefermentRequest.findOne({ _id: requestId }).populate(
      "userId",
      "name email"
    );
...
exports.deleteRequest = async (req, res) => {
  try {
    ...
    fileHelper.deleteFile(request.identityUrl);
    fileHelper.deleteFile(request.certificateUrl);
